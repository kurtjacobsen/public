// renolie-proxy-v3.6.44-p212b2-t23.mjs
...
app.get('/healthz', (_req,res)=> res.json({ ok:true, name:'renolie-proxy', version:'v3.6.44-p212b2-t23', pdf_engine: pdfEngine, pdf_mounted: pdfMounted, enc:'&aelig; &oslash; &aring; &AElig; &Oslash; &Aring; &deg;', ts:new Date().toISOString().replace('T',' ').replace('Z',' UTC') }));
app.get('/pdf/healthz', (_req,res)=> res.json({ ok:true, router:(pdfMounted?`pdf-${pdfEngine}`:'unmounted') }));
mountChatRoutes(app);
(async()=>{ try{ if(pdfEngine==='c23c'){ const { mountPdfRoutes } = await import('./routes-pdf-c23c.v2.mjs'); mountPdfRoutes(app); pdfMounted=true; } else { const { mountPdfRoutes } = await import('./routes-pdf-mock-v1.fix1.mjs'); mountPdfRoutes(app); pdfMounted=true; } } catch(e){ console.error('[pdf-mount] error:', e); pdfMounted=false; } })();
