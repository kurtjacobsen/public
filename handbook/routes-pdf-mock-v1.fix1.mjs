// routes-pdf-mock-v1.fix1.mjs
// ASCII-only. T2.1 mock. Endpoints: GET /pdf/healthz, POST /pdf/make, GET /pdf/echo

export function mountPdfRoutes(app) {
  app.get('/pdf/healthz', (_req, res) => res.json({ ok: true, router: 'pdf-mock' }));

  app.get('/pdf/echo', (req, res) => {
    res.json({ ok: true, demo: String(req.query.demo || '') });
  });

  app.post('/pdf/make', async (req, res) => {
    try {
      const body = req.body || {};
      const mode = body.rm_b64 ? 'rm_b64' : (body.recipe_markdown ? 'markdown' : 'none');

      // Tiny valid 1-page PDF as data URL (placeholder)
      const pdfUrl = 'data:application/pdf;base64,';
      res.json({ ok: true, mode, pdfUrl });
    } catch (e) {
      res.status(500).json({ ok: false, error: String(e && e.message || e) });
    }
  });
}
