// renolie-proxy-v3.6.49-p212b2-t26.mjs
// Mounts routes-pdf-c23c.v4d.mjs; runs background /tmp cleanup loop (interval via PROXY_CLEAN_INTERVAL_MIN).
import fs from 'fs';
...
function startCleaner(){
  ...
  // run once on start
  setTimeout(proxyCleanup, 5000);
}

let pdfEngine=(process.env.PDF_ENGINE||'mock').toLowerCase(); let pdfMounted=false;
app.get('/healthz',(_req,res)=>res.json({
  ok:true,name:'renolie-proxy',version:'v3.6.49-p212b2-t26',
