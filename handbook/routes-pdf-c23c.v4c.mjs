...
export function mountPdfRoutes(app) {
  app.get('/pdf/healthz', (_req, res) => {
    ...
    const info = listTmp();
    res.json({ ok:true, router:'pdf-c23c', impl, ttlMin: info.ttlMin, tmp: TMP_DIR, cleanup: { count: info.count, expired: info.expired } });
  });
  app.get('/pdf/echo', (req, res) => res.json({ ok:true, demo:String(req.query.demo || ''), ts: nowStr() }));
